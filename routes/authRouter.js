...
router.post('/login',
    body('email').trim().notEmpty().withMessage("Email isrequired!").isEmail().withMessage("Invalid Email address!"),
    body('password').trim().notEmpty().withMessage("Password isrequired!"),
    (req, res, next) => {
...
router.post('/forgot',
    ...
    (req, res, next) => {
        let message = getErrorMessage(req);
        if (message) {
            return res.render('forgot-password', { memssage })
        }
        next();
    },
